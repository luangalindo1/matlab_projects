...
%%%% QUESTAO 2E - Método de Newton desacoplado rápido na forma polar
...
% Aqui instancia-se vetores, para alocar as respostas
Vbarra2(iter) = V2;
ang2(iter) = teta2;
iteracao(iter) = iter;

% Jacobiano
J = [-V1*V2*B(2,1)*cos(teta2) 0; 0 B(2,1)*V1*cos(teta2)+2*V2*B(2,2)];
Jinv = inv(J);
...
Newton_Raphson_desacoplado_rapido = table(iteracao, Vbarra2, angle2)
